...
def _master_format_rules(row: dict[str, Any], value_cols: list[str]) -> dict[str, str]:
    """Build per-column format rules from a Master row.

    Prefers a `format_rules` JSON cell; otherwise applies a single `format`
    code to every value column.
    """
    rules_raw = _as_text(row.get("format_rules", ""))
    if rules_raw:
        try:
            parsed = json.loads(rules_raw)
            if isinstance(parsed, dict):
                return {_as_text(k): _as_text(v) for k, v in parsed.items()}
        except json.JSONDecodeError:
            pass
    fmt = _as_text(row.get("format", "")).lower()
    if fmt in _VALID_FORMAT_CODES:
        return {col: fmt for col in value_cols}
    return {}
...
def _pivot_long_to_wide(
    ...
    """Pivot one indicator's long/tidy rows into the wide frame the renderer expects.

    Row/category order and series/column order are preserved by first appearance.
    """
    if sub_df.empty:
        return pd.DataFrame()
    categories = list(dict.fromkeys(sub_df[category_col].tolist()))
    if series_col and series_col in sub_df.columns:
        series = list(dict.fromkeys(sub_df[series_col].tolist()))
        wide = sub_df.pivot_table(
            index=category_col, columns=series_col, values=value_col, aggfunc="first"
        )
        wide = wide.reindex(index=categories, columns=series)
    else:
        wide = sub_df.set_index(category_col)[[value_col]]
        wide = wide.reindex(index=categories)
        wide.columns = ["Value"]
    wide = wide.reset_index()
    label = x_name or category_col or "Category"
    wide = wide.rename(columns={category_col: label})
    wide.columns = [("Value" if str(c).strip() == "" else str(c)) for c in wide.columns]
    return wide


def _load_master_workbook(source_path: Path) -> WorkbookModel:
    """Load the two-sheet format: a `Master` config index + a stacked `Data` sheet.

    `Master`: one row per indicator with config columns (object_id, object_type,
    caption, figure_type, x_col, y_cols, axis titles, format(_rules), source_*).
    `Data`: tidy rows tagged by object_id, with category/series/value columns.
    """
    master = _read_excel_with_headers(source_path)["Master"].copy()
    data = _read_excel_with_headers(source_path)["Data"].copy()
    master.columns = [str(c).strip() for c in master.columns]
    data.columns = [str(c).strip() for c in data.columns]

    dcols = {c.lower(): c for c in data.columns}
    oid_c = dcols.get("object_id")
    cat_c = dcols.get("category") or dcols.get("row") or dcols.get("x")
    ser_c = dcols.get("series") or dcols.get("column")
    val_c = dcols.get("value")
    ...
    for order, raw in enumerate(master.to_dict("records")):
        ...
        x_col = _as_text(row.get("x_col"))
        wide = pd.DataFrame()
        if oid_c and cat_c and val_c:
            sub = data[data[oid_c].map(_as_text) == object_id]
            wide = _pivot_long_to_wide(sub, cat_c, ser_c, val_c, x_col)
        data_sheet = f"__data__{object_id}"
        data_frames[data_sheet] = wide
        value_cols = [str(c) for c in wide.columns[1:]] if not wide.empty else []

        section_tag = _as_text(row.get("section_tag")) or (
            object_id.split("-", 1)[1] if "-" in object_id else object_id
        )
        registry[object_id] = RegistryRecord(
            object_id=object_id,
            object_type=object_type,
            label=_as_text(row.get("label", object_id)) or object_id,
            caption=_as_text(row.get("caption")),
            data_sheet=data_sheet,
            enabled=True,
            section_tag=section_tag,
            order_index=_as_int(row.get("order_index", order + 1), default=order + 1),
        )

        format_rules = _master_format_rules(row, value_cols)
        if object_type == "figure":
            ...
            sdl_raw = row.get("show_data_labels", "")
            figure_specs[object_id] = FigureSpec(
                object_id=object_id,
                figure_type=_normalize_figure_type(row.get("figure_type", "line"), default="line"),
                x_col=x_col,
                y_cols=_parse_string_list(row.get("y_cols", "")),
                x_axis_title=_as_text(row.get("x_axis_title", x_col)),
                y_axis_title=_as_text(row.get("y_axis_title", "")),
                start_at_zero=_as_bool(row.get("start_at_zero", False)),
                hover_suffix=_as_text(row.get("hover_suffix", "")),
                pivot_for_chart=pivot_for_chart,
                show_data_labels=(
                    _as_bool(sdl_raw, default=False) if _as_text(sdl_raw) != "" else None
                ),
            )
        else:
            table_specs[object_id] = TableSpec(
                ...
                has_multilevel_headers=_as_bool(row.get("has_multilevel_headers", False)),
                format_rules=format_rules,
                row_label_col=x_col or (str(wide.columns[0]) if not wide.empty else ""),
            )

        source_specs[object_id] = SourceSpec(
            object_id=object_id,
            table_id=_as_text(row.get("source_table_id") or row.get("table_id")),
            url=_as_text(row.get("source_url") or row.get("url")),
            data_year=_as_int(row.get("source_data_year", row.get("data_year", 2023)), default=2023),
            estimate_type=_as_text(row.get("source_estimate_type", row.get("estimate_type", "5-Year Estimates")))
            or "5-Year Estimates",
            citation_month=_as_text(row.get("source_citation_month", row.get("citation_month", "April")))
            or "April",
            citation_year=_as_int(row.get("source_citation_year", row.get("citation_year", 2025)), default=2025),
            custom_text=_as_text(row.get("source_custom_text") or row.get("custom_text")),
        )
...
def _looks_like_master_workbook(source_path: Path, sheet_names: set[str]) -> bool:
    if not {"Master", "Data"}.issubset(sheet_names):
        return False
    try:
        header = pd.read_excel(source_path, sheet_name="Master", nrows=0)
    except Exception:
        return False
    return "object_id" in {str(c).strip().lower() for c in header.columns}
...
def load_cha_workbook(workbook_path: str | Path) -> WorkbookModel:
    ...
    if _looks_like_master_workbook(source_path, sheet_names):
        return _load_master_workbook(source_path)
    required = {"_registry", "_figure_specs", "_table_specs"}
